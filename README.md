...
本项目默认绑定作者自有域名（`pastro.ndjp.net`，见 `public/CNAME`），Fork 后需先调整三处配置，否则部署会失败或访问到错误域名：

1. **Fork 仓库**：在本仓库页面右上角点 **Fork**，生成 `<你的用户名>/pAstro`。
2. **删除 `public/CNAME`**：该文件写的是作者的域名，Fork 后留着会让 Pages 误绑域名导致 404。若你要用自定义域名，删除后按下方「绑定自定义域名」用你自己的域名重新创建。
3. **改 `astro.config.mjs`**：把 `site` / `base` 改成你的 Pages 地址：

   ```js
   export default defineConfig({
     site: "https://<你的用户名>.github.io",
     base: "/pAstro",        // 仓库名，注意大小写要和 Fork 出的仓库名一致
     // ...
   });
   ```

   - 如果 Fork 后改了仓库名（如改成 `nav`），`base` 也要同步改成 `/nav`。
   - 若部署到用户主页（仓库名为 `<用户名>.github.io`），`base` 留空 `""`。

4. **创建部署工作流**：本项目未内置 `.github/workflows`，需自行新建文件 `.github/workflows/deploy.yml`，内容如下：
...
5. **启用 Actions**：Fork 出的仓库默认会禁用 Actions。进入仓库 **Actions** 标签页，点击 **I understand my workflows, go ahead and enable them**。
6. **配置 Pages 来源**：**Settings** → **Pages** → **Build and deployment** 的 **Source** 选 **GitHub Actions**。
7. **推送并触发**：把上述改动 `git push` 到 `main` 分支，Actions 会自动构建部署。
8. **访问**：约 1-2 分钟后在 `https://<你的用户名>.github.io/PAstro/` 查看。
